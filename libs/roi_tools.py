...
def ptvr_to_rims_filename(filename, platform, deployment, camera, delim='_',ext = '.tif', timestamp_delta=None):
    ...
    frame_number = int(filename.split('-')[3])
    roi_number = int(filename.split('-')[4])
    left = int(filename.split('-')[5])
    top = int(filename.split('-')[6])
    width = int(filename.split('-')[7])
    height = int(filename.split('-')[8].split('.')[0]) 
    
    output_name = platform.upper()
    output_name += deployment + delim
    output_name += timestring + delim
    output_name += camera + delim
    output_name += str(frame_number) + delim
    output_name += str(roi_number) + delim
    output_name += str(left) + delim
    output_name += str(top) + delim
    output_name += "0" + delim
    output_name += str(width) + delim
    output_name += str(height) + delim
    output_name += "0"
    output_name += ext
